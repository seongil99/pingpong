...
class ImpactEffect {
    ...
    update() {
        ...
        for (const impact of this.impacts) {
            const elapsed =
                (currentTime - impact.startTime) / this.settings.duration;
            const expandFactor = Math.min(
                elapsed * this.settings.expandSpeed,
                1
            );

            impact.rings.forEach((ring, ringIndex) => {
                const positions =
                    ring.object.geometry.attributes.position.array;
                const baseRadius =
                    ring.radius +
                    (this.settings.maxRadius - ring.radius) * expandFactor;
                ...
                for (let i = 0; i < positions.length; i += 3) {
                    ...
                    const angle =
                        ...
                        (ring.rotationOffset +
                            this.settings.rotationSpeed * elapsed);
                    ...
                    positions[i] =
                        impact.position.x + Math.cos(angle) * currentRadius;
                ...
                // 회전
                ring.rotationOffset +=
                    this.settings.rotationSpeed * (ringIndex + 1);
    ...
    onKeyDown(event) {
        ...
        if (!this.secondPlayer && (key === "A" || key === "D")) {
            ...
            player.material.color.set(
                this.initColor[
                    nowIndexColor >= this.initColor.length - 1
                        ? 0
                        : nowIndexColor + 1
                ]
            );
    ...
    setupSocketListeners() {
        ...
        this.socket.on("data", (gameState) => {
            ...
            if (gameState.type === "gameState") {
                ...
                this.gameStart = WAIT_GAME;
                if (
                    gameState.sound == "ballToWall" &&
                    this.audio.sounds.has("ball")
                ) {
