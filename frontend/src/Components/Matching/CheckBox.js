...
function createFormSwitch(socket) {
    ...
    const formCheckInput = createElement("input", {
        ...
        events: {
            change: (event) => {
                switchStatusText.textContent = event.target.checked
                    ? "공두개"
                    : "기본";
            },
        },
                ...
                click: async () => {
                    ...
                    if (socket) {
                        ...
                        if (localStorage.getItem("matchType") === "Pve") {
                            ...
                            const gameId = await PvpRequest(
                                formCheckInput.checked
                            );
                            const modalElement =
                                document.getElementById("modal-tartget");
                            // 부모 엘리먼트를 얻기
